...
const TypingScreen = ({ initialTime, gameMode, targetWordCount, selected }) => {
    ...
    const calculateGameStats = useCallback(() => {
        ...
        let wpm;
        if (gameMode === 'time') {
            wpm = (correctWords / (initialTime / 60));
        } else {
    ...
    }, [isFocused, handleKeyDown, gameOver]);
    ...
    const renderGameStats = useCallback(() => (
        // Get game stats, send them as a json to server side to parse. Need to implement post request.
    ...
    const handleSubmitStats = useCallback(async () => {
        try {
            const userData = localStorage.getItem('user');
            const response = await axios.post(USERSTATS_URL,
                JSON.stringify({ gameStats, userId: userData.userId }), 
                {
            ...
            if (response) {
                console.log('Stats successfully submitted:', response.data);
